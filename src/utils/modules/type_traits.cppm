...
private:
  ...
  template <typename Callable_, typename... Args> struct FunctionArgTypes<Callable_(Args...)> {
  ...
  template <typename Callable_> struct ArgTypes_<Callable_, CallableKind::Function> {
    using Types = typename FunctionArgTypes<Callable_>::Types;
    constexpr static auto isCallableConst = true;
  };
  template <typename Callable_> struct ArgTypes_<Callable_, CallableKind::Lambda> {
    using Types = typename LambdaOrMethodArgTypes<decltype(&Callable_::operator())>::Types;
    constexpr static auto isCallableConst = LambdaOrMethodArgTypes<decltype(&Callable_::operator())>::isCallableConst;
  };
  template <typename Callable_> struct ArgTypes_<Callable_, CallableKind::Method> {
    using Types = typename LambdaOrMethodArgTypes<Callable_>::Types;
    constexpr static auto isCallableConst = LambdaOrMethodArgTypes<Callable_>::isCallableConst;
  };

  template <class Callable_> auto constexpr static GetCallableKind() -> CallableKind {
  ...
  constexpr static auto GenerateFunctionType(std::integer_sequence<size_t, Indices...>) -> std::function<
      typename CallableTraits<Callable_>::ReturnType(typename CallableTraits<Callable_>::template ArgType<Indices>...)>;
...
public:
  ...
  using std_function = decltype(GenerateFunctionType<RawCallable>(std::make_index_sequence<nArguments>{}));
  template <size_t n> constexpr static bool isLValueReference = std::is_lvalue_reference_v<Type<n>>;
  template <size_t n> constexpr static bool isRValueReference = std::is_rvalue_reference_v<Type<n>>;
  template <size_t n> constexpr static bool isReference = std::is_reference_v<Type<n>>;
  template <size_t n> constexpr static bool isValue = !isReference<n> && !std::is_void_v<Type<n>>;
  template <size_t n> constexpr static bool isConst = std::is_const_v<std::remove_reference_t<Type<n>>>;
