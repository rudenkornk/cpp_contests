...
auto inline missing_numbers(std::vector<unsigned> const &nums)
  ...
  unsigned const half_sum = boost::accumulate(
      nums, 0,
      [half_missing_sum](unsigned left,
                         unsigned right) noexcept -> unsigned int {
        right = (right <= half_missing_sum) ? right : 0;
        return left + right;
      });
  unsigned const total_half_sum =
      ((half_missing_sum + 1) * half_missing_sum) / 2;
